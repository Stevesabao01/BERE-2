...
void exibirCarrinho() {
    ...
    for (int i = 0; i < NUM_PRODUTOS; i++) {
        if (carrinho[i] > 0) {
            printf("Produto %s: R$%.2f x %d = R$%.2f\n", produtos[i].nome, produtos[i].preco, carrinho[i], produtos[i].preco * carrinho[i]);
...
int main() {
    int escolha, submenuProdutos, submenuLimpeza, submenuAlimentos, submenuPadaria, submenuCarrinho, submenuPagamentos;
    int quantidade[NUM_PRODUTOS] = {0};
    float carrinho[NUM_PRODUTOS] = {0};
    float desconto = 0.1;
    float valorGastoPorSetor[NUM_SETORES] = {0};
                    ...
                    switch (submenuProdutos) {
                        case 1:
                            do {
                                ...
                                if (submenuLimpeza >= 1 && submenuLimpeza <= 4) {
                                    printf("Informe a quantidade: ");
                                    scanf("%d", &quantidade[submenuLimpeza - 1]);
                                    adicionarCarrinho(submenuLimpeza - 1, quantidade[submenuLimpeza - 1]);
                                } else if (submenuLimpeza != 5) {
                        ...
                        case 2:
                            do {
                                ...
                                if (submenuAlimentos >= 1 && submenuAlimentos <= 5) {
                                    printf("Informe a quantidade: ");
                                    scanf("%d", &quantidade[submenuAlimentos + 4]);
                                    adicionarCarrinho(submenuAlimentos + 4, quantidade[submenuAlimentos + 4]);
                                } else if (submenuAlimentos != 6) {
                                    printf("Opcao invalida!\n");
                                }
                            } while (submenuAlimentos != 6);
                            break;
                        case 3:
                            do {
                                ...
                                if (submenuPadaria >= 1 && submenuPadaria <= 4) {
                                    printf("Informe a quantidade: ");
                                    scanf("%d", &quantidade[submenuPadaria + 9]);
                                    adicionarCarrinho(submenuPadaria + 9, quantidade[submenuPadaria + 9]);
                                } else if (submenuPadaria != 5) {
                                    printf("Opcao invalida!\n");
                                }
